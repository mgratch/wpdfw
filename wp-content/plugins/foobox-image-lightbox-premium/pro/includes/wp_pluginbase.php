...
if (!class_exists('wp_pluginbase_v2_6_2')) {

	require_once 'wp_pluginbase_v2_4_utils.php';

    abstract class wp_pluginbase_v2_6_2 {
		...
		/**
		 * @var wp_pluginbase_v2_4_utils
		 */
		private $_utils = false;
        ...
        function load_dependancies() {
			$this->_utils = new wp_pluginbase_v2_4_utils();
        }
        ...
        function check_wp_version($ver) {
            ...
            if (version_compare($wp_version, $ver) < 0) {
                throw new Exception("This plugin requires at least version $ver of WordPress. You are running an older version ($wp_version). Please upgrade!");
            }
        }
        ...
        function init() {
            ...
            if (is_admin()) {
                ...
                do_action($this->plugin_slug . '_admin_init', $this);
                ...
                do_action($this->plugin_slug . '_frontend_init', $this);
        ...
        function get_current_post_type() {
            ...
            elseif ($typenow)
                $get_current_post_type = $typenow; //check the global $current_screen object - set in sceen.php
            elseif ($current_screen && $current_screen->post_type)
                $get_current_post_type = $current_screen->post_type; //lastly check the post_type querystring
        ...
        function admin_add_error_message($message, $session = false) {
            if ($session && isset($_SESSION)) {
				...
				if (isset($_SESSION[$this->plugin_slug . '_admin_errors'])) {
        ...
        function admin_show_messages() {
            ...
            if (isset($_SESSION) && isset($_SESSION[$this->plugin_slug . '_admin_errors'])) {
                ...
                if (is_array($session_error_messages)) {
                    foreach ($session_error_messages as $error) {
                        $this->admin_show_message($error, true);
                    }
        ...
        function admin_settings_init() {
            do_action($this->plugin_slug . '-admin_settings_init', $this);
        }
        ...
        function admin_settings_add($args = array()) {
			...
			$type = $id = $desc = $default = $placeholder = $choices = $class = $separator = $section = $title = null;
            ...
            if (!empty($tab)) {
                ...
                //add the tab
                $this->admin_settings_add_tab($tab_id, $this->_utils->to_title($tab));

                //add the section
                $section_id = $this->admin_settings_add_section_to_tab($tab_id, $section_id, $this->_utils->to_title($section));
            } else {
                //just add the section
                $this->admin_settings_add_section($section_id, $this->_utils->to_title($section));
            }
        ...
        function admin_settings_render($args = array()) {
			...
			$id = $type = $default = $class = $desc = $choices = $separator = $placeholder = null;
            ...
            switch ($type) {
                ...
                case 'checkbox':
                    ...
                    //echo '<input type="hidden" name="'.$this->plugin_slug.'[' . $id . ']" value="' . $default . '" />';
                ...
                default:
					...
					do_action($this->plugin_slug . '_custom_admin_settings_render', $args);
                    break;
            }
...
//            }
        ...
        function admin_settings_add_menu() {
            if ($this->has_settings) {
                ...
                add_options_page($settings_title, $settings_menu,
        ...
        function admin_settings_render_page() {
            ...
            if (file_exists($this->plugin_dir . "settings.php")) {
                require_once($this->plugin_dir . "settings.php");
            } else {
                //render the settings using our default page
                require_once($this->plugin_dir . "includes/wp_pluginbase_default_settings.php");

                $settings_summary = $this->apply_filters('settings_summary', '');
                $settings_title = $this->get_settings_title();

                wp_pluginbase_v2_6_default_settings::render($settings_title, $settings_summary, $this->plugin_slug, $this->_settings_tabs);
            }
        }
        ...
        function get_settings_menu() {
            ...
            return $this->apply_filters('settings_menu', $title);
        }
        ...
        function register_shortcodes() {
            do_action($this->plugin_slug . '_register_shortcodes', $this);
        }
		...
		function get_options() {
			$options = is_network_admin() ?
				get_site_option($this->plugin_slug) :
				wp_parse_args( get_option($this->plugin_slug), get_site_option($this->plugin_slug) );
        ...
        function save_option($key, $value) {
            ...
            if (!$options) {
				...
				if (is_network_admin())
					add_site_option($this->plugin_slug, array($key => $value));
				else
