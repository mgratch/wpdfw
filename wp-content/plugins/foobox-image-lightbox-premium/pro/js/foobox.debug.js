	...
	FooBox.Modal = function (instance) {
		...
		this.setup = {
			...
			html: function () {
				...
				$inner.append('<a class="fbx-prev fbx-btn-transition fbx-btn-shadow"></a>');

				$inner.append('<div class="fbx-credit"><a target="_blank" href=""><em></em><span> FooBox</span></a></div>');
				$inner.append('<span class="fbx-count" />');
				$inner.append('<a class="fbx-next fbx-btn-transition fbx-btn-shadow"></a>');
				$inner.append('<a class="fbx-close fbx-btn-transition fbx-btn-shadow"></a>');
...
(function ($, FooBox) {
	...
	FooBox.Fullscreen = function (instance) {
		...
		this.handlers = {
			 ...
			 */
			setupHtml: function (e) {
				e.fb.modal.find('.fbx-inner').append('<a class="fbx-fullscreen-toggle fbx-btn-transition fbx-btn-shadow"></a>');
			},
			/**
			 * Handles the foobox.setupOptions event allowing this addon to set its initial starting state.
			 * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
		...
		this.handlers = {
			...
			setupHtml: function (e) {
				...
				e.fb.modal.find('.fbx-inner')
					...
					.append('<a class="fbx-play fbx-btn-transition fbx-btn-shadow"></a>');
			},
			/**
			 * Handles the foobox.setupOptions event allowing this addon to set its initial starting state.
			 * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
	...
	FooBox.Social = function (instance) {
		...
		this.handlers = {
			...
			setupHtml: function (e) {
				...
				e.fb.modal.find('.fbx-inner').append($social).append('<a class="fbx-social-toggle fbx-btn-transition fbx-btn-shadow"></a>');
			},
			/**
			 * Handles the foobox.setupOptions event allowing this addon to set its initial starting state.
			 * @param {jQuery.Event} e - A jQuery.Event object augmented with additional FooBox properties.
		...
		this.update = function(){
			...
			if ($.inArray(item.type, o.social.excludes) === -1){
				...
				if (o.social.mailto == true){
					var $mailto = $('<a/>', {href: 'mailto:?subject=' + encodeURIComponent(item.title) + '&body=' + encodeURIComponent(item.description + ' ' + (item.deeplink_url || item.url)), 'class': 'fbx-email', rel: 'nofollow', target: '_blank'});
