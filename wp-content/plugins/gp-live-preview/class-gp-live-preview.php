...
class GP_Live_Preview extends GWPerk {
	...
	public function display_preview_link() {
		...
		<div id="gplp-live-preview-template" style="display:none;">
			...
			<li class="gf_form_toolbar_preview gf_form_toolbar_live_preview">
				<a style="position:relative" id="gp-live-preview" class="gp-live-preview-link" href="<?php echo add_query_arg( 'id', $form_id, $this->get_preview_url() ); ?>" target="_blank">
	...
	public function get_live_preview_options() {
		return array(
			'ajax' => array(
				...
				'tooltip' => sprintf( '<h6>%s</h6> %s', __( 'Enable AJAX', 'gp-live-preview' ), __( 'Live Preview will be loaded with AJAX enabled.', 'gp-live-preview' ) ),
			),
			'showhidden' => array(
			...
			'unrequire' => array(
				...
				'tooltip' => sprintf( '<h6>%s</h6> %s', __( 'Unrequire', 'gp-live-preview' ), __( 'Live Preview will not require required fields be filled out to pass form validation.', 'gp-live-preview' ) ),
			),
		);
	}
	...
	public function maybe_load_preview_functionality() {
		...
		if( ( ! $this->is_live_preview() && ! $this->is_preview() ) || ! rgget( 'id' ) ) {
			return;
		}
	...
	public function documentation() {
		return array(
			'type' => 'url',
			'value' => 'http://gravitywiz.com/documentation/gp-live-preview-for-gravity-forms/'
		);
	}
